...
<instruction>
    <instructions>
        ...
        5. **[내용 보강]** `user-input`이 `literature-setting`과 직접 충돌하지 않더라도, `literature-setting`에 명시된 **캐릭터의 특징, 세계관 규칙, 고유 용어 등을 `user-input`에 자연스럽게 통합하여 내용을 더 풍부하고 구체적으로 만듭니다.** (예: 설정상 마법 사용 시 머리색이 변함 -> 마법 사용 장면에 '머리카락이 은빛으로 물들었다'는 묘사 추가)
        6. 수정된 결과물은 간결하고 정확해야 하며, 요청된 변경 사항(특히 설정 반영)을 완전히 반영해야 합니다.
        7. 폭력성/선정성 표현을 임의로 순화하지 말고 원작의 톤을 유지해야 합니다.
        8. 출력 결과에는 XML 태그(`<instruction>`, `<example>` 등)가 포함되지 않도록 합니다.
        9. 출력 결과에는 오직 **수정 및 보강이 완료된 최종 텍스트**만 포함되어야 합니다. 그 외의 설명이나 부연 설명은 절대 포함하지 마십시오.
    </instructions>
    <contexts>
    <context>
      <literature-setting>
