...
<instruction>
    <instructions>
        ...
        3. `literature-setting`에 명시되지 않은 새로운 인물 관계, 과거 사건, 중요한 사실 등을 절대 추론하여 만들어내서는 안 됩니다.
        4. `<part-of-the-work>`는 문체 파악에 참고하되, 최종 결과는 `<user_input>`과 `<how_polish>`에 집중합니다.
        5. `<how_polish>` 지침을 주의 깊게 해석하고 **요청 유형에 따라 접근 방식을 달리하세요**:
        ...
        6. 요청을 충족하고 설정과 일관성을 유지하는 데 필요한 **최소한의 수정을 기본 원칙**으로 하되, **표현 강화가 목적인 경우 (위 4-b 해당)** 원본의 의미를 해치지 않는 선에서 **더 효과적이고 문학적인 표현**을 적극적으로 사용할 수 있습니다. 원본의 어조와 핵심 의미는 유지하세요. 원본 표현을 **불필요하게 약화시키거나 평범하게 만들지 마십시오.** (예: '끼어들었다'의 갑작스러운 느낌을 유지)
        7. 폭력/선정성을 순화하지말아야 합니다.
        8. 출력 결과에는 XML 태그가 포함되지 않도록 합니다.
        9. 출력 결과에는 오직 user-input을 수정한 결과물만 포함되어야 합니다.
    </instructions>
    <literature-setting>
      {user_setting}
    </literature-setting>
