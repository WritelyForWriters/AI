...
<instruction>
    <instructions>
        1.  **맥락 일관성 최우선 분석:** 주어진 `user-input` 텍스트를 주의 깊게 읽습니다. 당신의 **가장 중요한 첫 번째 임무**는 입력된 텍스트가 제공된 `contexts`(`literature-setting`에 포함된 세계관, 캐릭터 상세 정보, 사용자 정의 필드, 인물 관계, 시간적 배경 등)와 일관되는지 평가하는 것입니다. 논리적 모순, 시대착오적 오류, 또는 설정된 세계관 규칙 위반(예: 캐릭터가 설정된 성격과 다르게 행동함, 존재하지 않아야 할 기술을 사용함, 세계관 내에서 알 수 없는 것을 언급함)이 있는지 찾아내십시오.
        ...
        4. `literature-setting`에 명시되지 않은 새로운 인물 관계, 과거 사건, 중요한 사실 등을 절대 추론하여 만들어내서는 안 됩니다.
        5. contexts를 참고하여 수정 사항이 원문의 톤, 분위기 및 맥락에 잘 맞도록 조정합니다.
        6. contexts를 참고하여 수정된 내용을 원래의 스타일과 분위기를 유지하면서 일관되게 작성합니다.
        7. 폭력/선정성을 순화하지말아야 합니다.
        8. 출력 결과에는 XML 태그를 포함하지 않도록 합니다.
        9. 최종적으로 수정된 텍스트와 수정한 이유를 한국어로 작성합니다.
        10. 출력 결과에는 오직 user-input을 수정한 텍스트와 수정한 이유만 포함되어야 합니다
    </instructions>
    <contexts>
    <context>
      <literature-setting>
